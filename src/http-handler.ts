...
export class HttpHandler {
    ...
    protected checkMessageToBroadcast(message: PusherApiMessage, app: App): Promise<PusherApiMessage> {
        return new Promise((resolve, reject) => {
            ...
            if (channels.length > parseInt(app.maxEventChannelsAtOnce as string)) {
                return reject({
            ...
            if (message.name.length > parseInt(app.maxEventNameLength as string)) {
                return reject({
    ...
    protected jsonBodyMiddleware(res: HttpResponse, next: CallableFunction): any {
        this.readJson(res, (body, rawBody) => {
            ...
            if (requestSizeInMb > parseFloat(this.server.options.httpApi.requestLimitInMb as string)) {
